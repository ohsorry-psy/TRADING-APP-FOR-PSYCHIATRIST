...
fig = make_subplots(
    ...
    row_heights=[0.7, 0.3],
    subplot_titles=(f"{symbol} 가격 + RSI 다이버전스 + 볼린저 밴드", "RSI")
)
...
if 'bb_bbm' in data.columns:
    fig.add_trace(go.Scatter(x=data.index, y=data['bb_bbh'], name='볼린저밴드 상단', line=dict(color='lightgray', dash='dot')), row=1, col=1)
    fig.add_trace(go.Scatter(x=data.index, y=data['bb_bbm'], name='볼린저밴드 중간', line=dict(color='gray', dash='dot')), row=1, col=1)
    fig.add_trace(go.Scatter(x=data.index, y=data['bb_bbl'], name='볼린저밴드 하단', line=dict(color='lightgray', dash='dot')), row=1, col=1)
